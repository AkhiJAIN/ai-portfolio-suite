...
const Crystal = () => {
  ...
  return (
    <Icosahedron ref={ref} args={[1.15, 0]}>
      {/* @ts-ignore */}
      <MeshTransmissionMaterial thickness={1.2} roughness={0.05} transmission={1} ior={1.4} chromaticAberration={0.06} backside color="#22d3ee" />
    </Icosahedron>
  );
};
...
export const Hero3D = () => {
  return (
    <Canvas camera={{ position: [0, 0, 5], fov: 45 }} style={{ position: "absolute", inset: 0, pointerEvents: "none" }} dpr={[1, 1.5]}>
      ...
      <directionalLight position={[5, 5, 5]} intensity={1.4} color="#3b82f6" />
      <directionalLight position={[-5, -3, 2]} intensity={0.7} color="#67e8f9" />
      <Suspense fallback={null}>
        ...
        <Float speed={2} rotationIntensity={1} floatIntensity={1.5}>
          <Octahedron args={[0.4, 0]} position={[1.9, 1.1, 0.5]}>
            {/* @ts-ignore */}
            <MeshTransmissionMaterial thickness={0.8} roughness={0.05} transmission={1} ior={1.5} color="#3b82f6" />
          </Octahedron>
        </Float>
        <Float speed={1.6} rotationIntensity={0.8} floatIntensity={1.4}>
          <Icosahedron args={[0.28, 0]} position={[-1.8, -1, 0.3]}>
            {/* @ts-ignore */}
            <MeshTransmissionMaterial thickness={0.6} roughness={0.05} transmission={1} ior={1.4} color="#67e8f9" />
          </Icosahedron>
        </Float>
        <SpinningTorus />
      </Suspense>
